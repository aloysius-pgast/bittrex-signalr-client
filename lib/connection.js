...
//-- Bittrex endpoints
// base url used for legacy endpoint
const BASE_PATH_LEGACY = 'socket.bittrex.com/signalr';
// base url used for new endpoint
const BASE_PATH_NEW = 'beta.bittrex.com/signalr';
...
//-- SignalR config
// hub used for legacy endpoint
const HUB_LEGACY = 'corehub';
// hub used for new endpoint
const HUB_NEW = 'c2';
...
/**
 * Returns an https url for legacy/new endpoints
 *
 * @param {path} url path
 * @return {string}
 */
_getHttpsUrl(path)
{
    if (this._legacy)
    {
        return `https://${BASE_PATH_LEGACY}/${path}`;
    }
    return `https://${BASE_PATH_NEW}/${path}`;
}

/**
 * Returns a wss url for legacy/new endpoints
 *
 * @param {path} url path
 * @return {string}
 */
_getWssUrl(path)
{
    if (this._legacy)
    {
        return `wss://${BASE_PATH_LEGACY}/${path}`;
    }
    return `wss://${BASE_PATH_NEW}/${path}`;
}

/**
 * Returns SignalR hubs for legacy/new endpoints
 *
 * @return {array}
 */
_getHubs()
{
    if (this._legacy)
    {
        return [{name:HUB_LEGACY}];
    }
    return [{name:HUB_NEW}];
}

/**
 * Return hub name for lagacy/new endpoints
 *
 * @return {string}
 */
_getHubName()
