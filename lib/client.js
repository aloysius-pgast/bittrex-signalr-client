...
constructor(options)
{
    ...
    // by default use legacy endpoints
    this._legacy = true;
    ...
    if (undefined !== options)
    {
        ...
        // legacy
        if (false === options.legacy)
        {
            this._legacy = false;
        }
        this._connectionOptions.legacy = this._legacy;
...
{
    // no support
    if (this._legacy || null === this._auth.key)
    {
        return false;
    }
...
_processChanges(changes, connect)
{
    ...
    if (undefined !== changes.resync)
    {
        _.forEach(changes.resync, (entry) => {
            if (this._legacy)
            {
                this._queryExchangeStateLegacy(entry.pair);
            }
            else
            {
                this._queryExchangeStateNew(entry.pair);
            }
        });
    }
 ...
 */
_queryExchangeStateLegacy(pair)
{
    // reset nounce
        ...
        switch (methodName)
        {
            case 'updateexchangestate':
                _.forEach(data.A, (entry) => {
                    this._processUpdateExchangeStateLegacy(entry);
                })
                break;
            case 'ue':
                _.forEach(data.A, (entry) => {
                    this._processUpdateExchangeStateNew(entry);
                })
                break;
            case 'updatesummarystate':
                _.forEach(data.A, (entry) => {
                    this._processUpdateSummaryStateLegacy(entry);
                })
                break;
            case 'us':
                _.forEach(data.A, (entry) => {
                    this._processUpdateSummaryStateNew(entry);
                })
                break;
            case 'uo':
                _.forEach(data.A, (entry) => {
...
/**
 * Process Market Delta for legacy endpoint
 */
 ...
 */
_processUpdateExchangeStateLegacy(data)
{
    ...
    if (missedCount > 1 || missedCount < 0)
    {
        ...
        this._queryExchangeStateLegacy(data.MarketName);
        return;
    }
...
/**
 * Process Summary Delta for legacy endpoint
 */
 ...
 */
_processUpdateSummaryStateLegacy(data)
{
...
/**
 * Process Market Delta for new endpoint
 */
 ...
 */
_processUpdateExchangeStateNew(d)
{
    this._decodeData(d, function(data){
        ...
        if (missedCount > 1 || missedCount < 0)
        {
            ...
            this._queryExchangeStateNew(data.M);
            return;
        }
        // ignore update since it's the same nounce as it's the same one as the full order book we last received
        else if (0 == missedCount)
...
/**
 * Process Summary Delta for new endpoint
 */
 ...
 */
_processUpdateSummaryStateNew(d)
{
    this._decodeData(d, function(data){
        // an error occurred
        if (undefined === data)
        ...
        _.forEach(data.D, (entry) => {
            ...
            let evt = {
                ...
                data:{
                    ...
                    timestamp: parseFloat(new Date(entry.T).getTime() / 1000.0) + this._timeOffset
                }
            }
            this.emit('ticker', evt);
        });
...
/**
 * Process Orders Delta for new endpoint
 */
...
/**
 * Decode data received from new endpoint :
 * 1) base64 decode
 * 2) gzip inflate
 */
_decodeData(d, cb)
